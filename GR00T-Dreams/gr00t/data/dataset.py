...
class LeRobotSingleDataset(Dataset):
    ...
    def get_language(
        ...
        if subkey_meta is None:
            fallback_subkey = None
            if ".annotation." in subkey:
                alt_subkey = subkey.replace(".annotation.", ".")
                if alt_subkey in annotation_meta:
                    fallback_subkey = alt_subkey
            else:
                parts = subkey.split(".", 1)
                if len(parts) == 2:
                    alt_subkey = f"{parts[0]}.annotation.{parts[1]}"
                    if alt_subkey in annotation_meta:
                        fallback_subkey = alt_subkey
            if fallback_subkey is not None:
                subkey = fallback_subkey
                subkey_meta = annotation_meta[subkey]
        assert (
            subkey_meta is not None
