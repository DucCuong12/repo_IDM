...
DEFAULT_MODEL_CONFIG = "idm/m2_bottle100/checkpoint-9500/config.json"
DEFAULT_MODEL_CKPT = "idm/m2_bottle100/checkpoint-9500/model.safetensors"
...
def infer_actions_from_frames(model, frames, device, embodiment_tag, denormalizer=None, siglip_processor=None):
    ...
    print(f'This has: {len(frames)} frames, so will infer {len(frames)-1} actions (frame pairs).')
    ...
    with torch.no_grad():
        for i in tqdm(range(len(frames) - 1), desc="Infer actions", leave=False):
            ...
            img2 = torch.from_numpy(processed_frames[i + 1]).float()
            ...
            # horizon=1, so we only need [B, 1, action_dim]
            dummy_actions = torch.zeros(B, 1, action_dim, device=device)
            ...
            sa_tokens = [_ACT_TOKEN] * 1  # 1 action token (horizon=1)
            ...
            # Get normalized action (model output is [B, Horizon, Dim] -> [1, 1, 26])
            normalized_action = action[0, 0].detach().cpu().numpy()
