...
def load_dataset_and_config(checkpoint_path, validation_dataset_path, video_indices):
    ...
    metadata_version_str = str(metadata_version)
    reverse_lookup = {tag.value: tag for tag in EmbodimentTag}

    if metadata_version_str in reverse_lookup:
        embodiment_tag = reverse_lookup[metadata_version_str]
    else:
        normalized_key = metadata_version_str.upper()
        embodiment_tag = EmbodimentTag.__members__.get(normalized_key, metadata_version_str)
